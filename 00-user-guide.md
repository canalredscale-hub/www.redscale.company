...
- Ultima aprovacao: a home institucional deve continuar em pagina unica, preservando a base atual de `Hero`, `Sobre`, `Servicos`, `CTA com imagem` e `Metodo`, e expandindo a narrativa com `Produtos`, `Redsights` e CTA final.
...
- Onde paramos: home com header, hero textual, faixa de video, `Sobre`, `Nossos Servicos`, `CTA com imagem`, `Metodo`, `Produtos`, `Redsights` e fechamento institucional em `#contato`, com a logica de pintura tipografica compartilhada em `app/frontend/paint-animations.css`.
...
6. CTA com imagem.
   ...
   Estilo atual: o frame usa cantos arredondados amplos, cobre toda a largura util da pagina e aplica uma sombra/overlay leve para preservar legibilidade; o squircle vermelho abre o bloco no canto superior esquerdo e o titulo usa `Aspekta` em escala grande, cor `#262626`, uma palavra por linha. No canto inferior direito entra uma copia do CTA do header, agora com o texto `Entre em contato conosco`.
...
7. Secao Metodo.
   ...
   Estilo atual: o titulo usa a mesma largura util de `502px`, o mesmo line-height `0.98` e a mesma regua horizontal do texto principal e do subtexto de `Sobre`; a expressao `Vem do resultado!` recebe sozinha a pintura tipografica em `#8C1F28`, com um ciclo dedicado de `16s`. Logo abaixo, o subtexto fica no mesmo grid-column e no mesmo alinhamento estrutural usados pelo bloco de apoio de `Sobre`, preservando o cinza institucional e a largura de leitura ja aprovada. A secao continua sem infografico `CAR` e sem video, funcionando apenas como bloco textual independente entre o CTA com imagem e `Produtos`.
