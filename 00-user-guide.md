...
- Ultima aprovacao: a home institucional deve continuar em pagina unica, preservando a base atual de `Hero`, `Sobre` e `Servicos`, e expandindo a narrativa com `Nosso Approach`, `Produtos`, `Redsights` e CTA final.
...
- Onde paramos: home com header, hero textual, faixa de video, `Sobre`, `Nosso Approach`, `Nossos Servicos`, `Produtos`, `Redsights` e fechamento institucional em `#contato`, com a logica de pintura tipografica compartilhada em `app/frontend/paint-animations.css`.
...
5. Secao Nosso Approach.
   Posicao: visualmente antecipada para iniciar aproximadamente `2cm` abaixo do ultimo dos dois blocos complementares de `Sobre`; o encaixe agora usa a altura real do bloco que terminar mais baixo, para que `Nosso Approach` faca parte do mesmo fechamento visual da secao sem alterar a cadencia das seguintes.
   Estrutura atual: label `Nosso Approach`, titulo principal sem texto de apoio e, logo abaixo, um corpo unico em largura total, com o infografico `Metodologia CAR` primeiro e o video local do carro logo abaixo.
   Papel atual: condensar o approach institucional da REDSCALE em contexto, acao e resultados, mantendo a leitura compacta e visualmente proxima da secao `Sobre`.
   Conteudo atual: titulo auxiliar `Metodologia CAR`, grafico triangular com tres faixas de cor e legenda lateral com `C / Contexto`, `A / Acao` e `R / Resultados`.
   Estilo atual: o titulo `Nos focamos em transparencia, criatividade e colaboracao para garantir que cada projeto esteja alinhado ao seu objetivo.` usa a mesma largura util de `502px`, o mesmo line-height `0.98` e agora tambem a mesma regua horizontal do bloco principal `Somos especialistas...`, mas sem mover o label `Nosso Approach`; o ajuste acontece apenas no titulo por meio de um offset dedicado, preservando o squircle e o marcador onde ja estavam. A pintura tipografica continua no padrao compartilhado de `7s` de repouso e `7s` de sustentacao completa, destacando `transparencia`, `criatividade`, `colaboracao`, `alinhado` e `seu objetivo` em `#8C1F28`. Abaixo dele, o infografico `CAR` se estende ate o recuo util da direita com cores `#044040`, `#8C1F28` e `#D92525`; logo abaixo entra o video `redscale-f1.mp4` em frame amplo, sem texto sobreposto e no mesmo idioma de preenchimento do video do hero.

6. Secao Nossos Servicos.
   Posicao: logo abaixo do fechamento da secao `Sobre`, abrindo com o novo espacamento global aproximado de `5cm`.
   Estrutura atual: mantem o bloco introdutorio em duas colunas usado em `Sobre`, com o marcador da secao (`squircle` e `Nossos Servicos`) no alinhamento anterior e apenas o texto principal recuado aproximadamente `5cm` para a direita; logo abaixo entra um paragrafo de apoio institucional e, cerca de `2cm` depois, uma pilha vertical interativa de quatro ofertas com divisorias finas, acompanhada por um shape proprio espelhado no lado esquerdo.
...
7. Secao Produtos.
   Posicao: abaixo de `Nossos Servicos`, respeitando o mesmo espacamento vertical global aproximado de `5cm`.
