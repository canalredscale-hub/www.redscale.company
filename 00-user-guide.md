...
- Ultima aprovacao: a home institucional deve continuar em pagina unica, preservando a base atual de `Hero`, `Sobre` e `Servicos`, e expandindo a narrativa com `Sobre o Metodo`, `Produtos`, `Redsights` e CTA final.
...
- Onde paramos: home com header, hero textual, faixa de video, `Sobre`, `Sobre o Metodo`, `Nossos Servicos`, `Produtos`, `Redsights` e fechamento institucional em `#contato`, com a logica de pintura tipografica compartilhada em `app/frontend/paint-animations.css`.
...
5. Secao Sobre o Metodo.
   Posicao: visualmente antecipada para iniciar aproximadamente `2cm` abaixo do ultimo dos dois blocos complementares de `Sobre`; o encaixe agora usa a altura real do bloco que terminar mais baixo, para que `Sobre o Metodo` faca parte do mesmo fechamento visual da secao sem alterar a cadencia das seguintes.
   Estrutura atual: label `Sobre o Metodo`, titulo principal sem texto de apoio e, logo abaixo, um corpo unico em largura total, com o infografico `CAR` primeiro e o video local do carro logo abaixo.
   ...
   Conteudo atual: grafico triangular com tres faixas de cor e legenda lateral com `C / Contexto`, `A / Acao` e `R / Resultados`.
   Estilo atual: o titulo `Nos focamos em transparencia, colaboracao e aplicacao correta de principios para garantir que o projeto esteja alinhado ao seu objetivo.` usa a mesma largura util de `502px`, o mesmo line-height `0.98` e a mesma regua horizontal do bloco principal `Somos especialistas...`, mas sem mover o label `Sobre o Metodo`; o ajuste acontece apenas no titulo por meio de um offset dedicado, preservando o squircle e o marcador onde ja estavam. A pintura tipografica segue com cadencia sequencial dedicada de `26s`, sempre iniciando a palavra seguinte apenas depois que a anterior conclui a entrada, destacando `transparencia`, `colaboracao`, `principios`, `projeto`, `alinhado` e `seu objetivo` em `#8C1F28`. Abaixo dele, o infografico `CAR` se estende ate o recuo util da direita com cores `#044040`, `#8C1F28` e `#D92525`, agora sem o subtitulo `Metodologia CAR`; logo abaixo entra o video `redscale-f1.mp4` em frame amplo, sem texto sobreposto e no mesmo idioma de preenchimento do video do hero.
