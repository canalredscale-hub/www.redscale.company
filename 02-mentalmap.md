...
### 2026-04-05T23:28:55.4872470-03:00 - Textos de apoio do Sobre passaram a quebrar mais 2cm antes novamente

- Fase impactada: Marco 3. Capacidades minimas do workflow `web_app`.
- Decisao aprovada: recuar mais aproximadamente `2cm` a quebra dos textos de apoio do bloco complementar do `Sobre`, preservando o alinhamento atual dos titulos.
...
### 2026-04-09T11:20:00-03:00 - Metodo virou secao independente abaixo de Servicos
...
### 2026-04-09T12:30:00-03:00 - CTA com imagem entrou entre Servicos e Metodo

- Fase impactada: Marco 3. Capacidades minimas do workflow `web_app`.
- Decisao aprovada: copiar `cta-img.jpg` de `D:\Imagens` para os assets do site, inserir um CTA com imagem aproximadamente `5cm` abaixo de `Nossos Servicos`, atualizar o CTA do header para `Entre em contato` e normalizar a regua horizontal de `Produtos` e `Redsights` pela mesma referencia de `Sobre`.
- Implementacao entregue: `app/frontend/assets/images/cta/cta-img.jpg` foi incorporada ao projeto; `app/frontend/index.html` atualizou o CTA do header, criou a secao `image-cta` com squircle, titulo `Fazemos, Repetimos, Melhoramos` e botao `Entre em contato conosco`, e preservou `Metodo` abaixo desse novo bloco; `app/frontend/styles.css` fixou a coluna de label das secoes em uma regua comum, adicionou o layout da imagem e manteve `Produtos` e `Redsights` alinhados ao mesmo eixo de `Sobre`; `00-user-guide.md`, `README.md` e `02-mentalmap.md` foram sincronizados.
- Proximo movimento coerente: validar no preview se a sobreposicao do texto no canto superior esquerdo permanece legivel em diferentes larguras e se o CTA inferior direito manteve presenca suficiente sobre a area clara da imagem.
